...
function transformUrl(url, outputFilename, sourceFilename, platformPath = _path.default) {
  ...
  const relative = platformPath.relative(
    platformPath.dirname(outputFilename), // Get the absolute path to the asset from the path relative to the JS file
      ...
      default:
        _stylis.default.use(null)((context, decl) => {
          ...
          if (context === STYLIS_DECLARATION && outputFilename) {
            ...
            return decl.replace(
              /\b(url\((["']?))(\.[^)]+?)(\2\))/g,
              (match, p1, p2, p3, p4) => p1 + transformUrl(p3, outputFilename, options.filename) + p4,
            )
          }
...
function transform(code, options) {
  ...
  const babelOptions =
    (_pluginOptions$babelO =
      pluginOptions === null || pluginOptions === void 0 ? void 0 : pluginOptions.babelOptions) !== null &&
    _pluginOptions$babelO !== void 0
      ? _pluginOptions$babelO
      : null // Parse the code first so babel uses user's babel config for parsing
  ...
  const babelFileResult = (0, _core.transformFromAstSync)(ast, code, {
    ...(babelOptions !== null && babelOptions !== void 0 && babelOptions.rootMode
      ? {
