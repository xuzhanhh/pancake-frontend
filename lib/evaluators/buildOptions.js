...
function buildOptions(filename, options) {
  ...
  const defaults = {
    ...
    plugins: [
      ...plugins.map((name) => require.resolve(name)), // We don't support dynamic imports when evaluating, but don't wanna syntax error
      // This will replace dynamic imports with an object that does nothing
      require.resolve('../dynamic-import-noop'),
    ],
  }
  const babelOptions = options !== null && options !== void 0 && options.babelOptions ? { ...options.babelOptions } : {} // Shallow copy the babel options because we mutate it later // If we programmatically pass babel options while there is a .babelrc, babel might throw
    ...
    babelOptions[field] = babelOptions[field]
      ? babelOptions[field].filter((item) => {
          ...
          if (
            ...
            name === require.resolve('../index') || // Also add a check for the plugin names we include for bundler support
          ...
          return !defaults[field].some(
            (
              it, // The default presets/plugins can also have nested arrays,
  ...
  return {
    ...
    plugins: [
      ...defaults.plugins, // Plugin order is first to last, so add the extra presets to end
