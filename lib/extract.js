    ...
    visitor: {
      Program: {
        enter(path, state) {
          ...
          const lazyDeps = state.queue.reduce((acc, { expressionValues: values }) => {
          ...
          if (expressionsToEvaluate.length > 0) {
            ;(0, _logger.debug)(
              ...
              originalLazyExpressions.map((node) =>
                typeof node !== 'string' ? (0, _generator.default)(node).code : node,
              ),
            )
            ;(0, _logger.debug)(
            ...
            const { code } = (0, _generator.default)(program)
