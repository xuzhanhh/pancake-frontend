...
function hoist(babel, ex) {
  const Identifier = (idPath) => {
    ...
    if (!binding) return
    const { scope, path: bindingPath, referencePaths } = binding // parent here can be null or undefined in different versions of babel
...
function CollectDependencies(babel, path, state, options) {
  ...
  ;(0, _logger.debug)(
    ...
    expressionValues.map((expressionValue) =>
      expressionValue.kind === _types.ValueType.VALUE ? expressionValue.value : 'lazy',
    ),
  )
