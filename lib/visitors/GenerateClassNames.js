...
function GenerateClassNames(babel, path, state, options) {
  ...
  slug =
    slug ||
    (0, _toValidCSSIdentifier.default)(
      `${displayName.charAt(0).toLowerCase()}${(0, _utils.slugify)(
        `${(0, _path.relative)(state.file.opts.root, state.file.opts.filename)}:${state.index}`,
      )}`,
  ...
  let className = predefinedClassName
    ? predefinedClassName
    : options.displayName
    ? `${(0, _toValidCSSIdentifier.default)(displayName)}_${slug}`
    : slug // The className can be defined by the user either as fn or a string
  ...
  if (typeof options.classNameSlug === 'string') {
    const { classNameSlug } = options // Variables that were used in the config for `classNameSlug`
