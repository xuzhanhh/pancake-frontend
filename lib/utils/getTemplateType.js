...
function getTemplateTypeByTag(t, path, localName, has) {
  ...
  if (t.isCallExpression(tag) && t.isIdentifier(tag.callee) && tag.arguments.length === 1 && tag.callee.name === localName && has(localName, ['@linaria/react','@pancakeswap/mp-styled-2','styled-components', 'linaria/react'])) {
  ...
  if (t.isMemberExpression(tag) && t.isIdentifier(tag.object) && t.isIdentifier(tag.property) && tag.object.name === localName && has(localName, ['@linaria/react','@pancakeswap/mp-styled-2','styled-components', 'linaria/react'])) {
    return {
      component: {
