...
function getTemplateTypeByTag(t, path, localName, has) {
  const { tag } = path.node // styled(Cmp)``
  ...
  if (
    ...
    has(localName, ['@linaria/react', '@pancakeswap/mp-styled-2', 'linaria/react'])
  ) {
...
function getTemplateType({ types: t }, path, state, libResolver) {
  var _cache$get

  if (!cache.has(path)) {
    const localName = state.file.metadata.localName || 'styled'

    const has = (identifier, sources) =>
      (0, _hasImport.default)(t, path.scope, state.file.opts.filename, identifier, sources, libResolver)
