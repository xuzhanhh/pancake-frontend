...
import { Slider } from '@binance/mp-components'
...
export default function RemoveLiquidity() {
  ...
  const { trackLiquidityRemove } = useHandleTrack()
  // tx sending
  ...
  const handleSelectCurrencyA = useCallback(
    (currency: Currency) => {
      if (currencyIdB && currencyId(currency) === currencyIdB) {
        liquidityDispatch({ type: 'setCurrency', currency1: currencyId(currency), currency2: currencyIdA })
      } else {
        liquidityDispatch({ type: 'setCurrency', currency1: currencyId(currency), currency2: currencyIdB })
      }
    },
  ...
  const handleSelectCurrencyB = useCallback(
    (currency: Currency) => {
      if (currencyIdA && currencyId(currency) === currencyIdA) {
        liquidityDispatch({ type: 'setCurrency', currency1: currencyIdB, currency2: currencyId(currency) })
      } else {
        liquidityDispatch({ type: 'setCurrency', currency1: currencyIdA, currency2: currencyId(currency) })
      }
    },
    [currencyIdA, currencyIdB],
  )
  ...
  const [onPresentRemoveLiquidity] = useModal(
    <TransactionConfirmationModal
      title={`${t('You will receive')} ${currencyA.symbol}-${currencyB.symbol} LP`}
  ...
  return (
    <ErrorBoundary name="removeLiquidity">
      <view>
        <AppBody>
          <AppHeader
            // backTo="/liquidity"
